...
class ActionValidator:
    ...
    PERMITTED_ACTIONS: Dict[str, List[PermittedAction]] = {
        ...
        "SyntheticDataAgent": [
            PermittedAction(
                "SyntheticDataAgent", "generate_synthetic", ["product_data"], "write"
            ),
            PermittedAction(
                "SyntheticDataAgent", "create_competitor", ["product_data"], "write"
            ),
        ],
        "AnalysisAgent": [
            PermittedAction(
                "AnalysisAgent", "extract_benefits", ["product_data"], "read"
            ),
            PermittedAction(
                "AnalysisAgent", "generate_questions", ["product_data"], "read"
            ),
        ...
        "ValidationAgent": [
            PermittedAction(
                "ValidationAgent", "validate", ["analysis_results"], "read"
            ),
            PermittedAction("ValidationAgent", "check_quality", [], "read"),
        ],
        "GenerationAgent": [
            PermittedAction(
                "GenerationAgent", "generate", ["analysis_results"], "write"
            ),
            PermittedAction(
                "GenerationAgent", "create_faq", ["generated_questions"], "write"
            ),
            PermittedAction(
                "GenerationAgent", "create_product_page", ["product_data"], "write"
            ),
            PermittedAction(
                "GenerationAgent", "create_comparison", ["comparison_data"], "write"
            ),
        ],
        "VerifierAgent": [
    ...
    def validate_action(
        self, agent: str, action: str, context: "AgentContext"
    ) -> ValidationResult:
        """
        ...
        if action in self.UNIVERSAL_ACTIONS:
            return self._create_result(
                True, f"{action} is a universal action", agent, action
            )
        ...
        if not permitted:
            ...
            return self._create_result(
                True, "Unknown agent, no rules defined", agent, action
            )
    ...
    def check_data_grounding(self, output: Any, context: "AgentContext") -> List[str]:
        """
    ...
    def detect_fabrication(self, text: str, source_data: Dict[str, Any]) -> List[str]:
        """
