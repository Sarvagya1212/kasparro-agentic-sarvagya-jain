...
DATA_AGENT_PROPOSAL = PromptTemplate(
    ...
    system_prompt="""You are the Data Agent, responsible for loading and validating product data.
Your role is to determine if data loading is needed and propose appropriate actions.
...
Based on this analysis, should you act?""",
    examples=[
        PromptExample(
            ...
            output_text='{"should_act": true, "action": "load_data", "confidence": 0.95, "reason": "No product data - loading is required"}',
            explanation="Data loading is the first step, and no data exists yet"
        ),
        PromptExample(
            input_text="Product Data: Loaded, Analysis complete",
            output_text='{"should_act": false, "action": "none", "confidence": 0.1, "reason": "Data already loaded, workflow past data phase"}',
...
DELEGATOR_PROPOSAL = PromptTemplate(
    ...
    system_prompt="""You are the Delegator Agent, a project manager coordinating analysis tasks.
Your role is to decompose complex analysis into subtasks and delegate to specialized workers.
Focus on task dependencies, optimal ordering, and worker capabilities.""",
    user_template="""## Current State
...
Determine next analysis task and assign to appropriate worker.""",
    examples=[
        PromptExample(
            ...
            output_text='{"should_act": true, "action": "delegate_benefits", "confidence": 0.9, "reason": "Benefits extraction is foundation for other analysis", "worker": "BenefitsWorker"}',
            explanation="Benefits should be extracted first as other tasks depend on it"
        )
    ],
    output_format="""Respond in JSON:
{
...
GENERATION_PROPOSAL = PromptTemplate(
    ...
    system_prompt="""You are the Generation Agent, responsible for creating final content.
Your role is to evaluate when analysis is complete and generate output content.
...
VERIFIER_PROPOSAL = PromptTemplate(
    ...
    system_prompt="""You are the Verifier Agent, an independent auditor ensuring quality.
Your role is to validate outputs, check for errors, and ensure safety compliance.
...
REFLECTION_PROMPT = PromptTemplate(
    ...
    agent_type="Any",
    system_prompt="""You are reflecting on your recent output to identify issues and improvements.
...
class PromptRegistry:
    ...
    def __init__(self):
        self._templates: Dict[str, Dict[str, PromptTemplate]] = {}  # id -> version -> template
    ...
    def get_for_agent(
        ...
        for versions in self._templates.values():
            for template in versions.values():
                if template.agent_type == agent_type and template.prompt_type == prompt_type:
                    return template
        return None
...
def get_agent_prompt(
    ...
    if template:
        return (
            ...
            template.render(context, include_examples=True)
        )

    # Fallback
    return (
        f"You are the {agent_type}.",
