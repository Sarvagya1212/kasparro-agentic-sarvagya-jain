...
class AgentGoalManager:
    ...
    def evaluate_progress(self, context: "AgentContext") -> Dict[str, float]:
        ...
        for goal in self._goals.values():
            if goal.is_terminal():
                progress_map[goal.id] = 1.0 if goal.status == GoalStatus.COMPLETED else 0.0
                continue
    ...
    def get_progress_summary(self) -> Dict[str, Any]:
        ...
        failed = sum(1 for g in self._goals.values()
                    if g.status == GoalStatus.FAILED)
        in_progress = sum(1 for g in self._goals.values()
                         if g.status == GoalStatus.IN_PROGRESS)
    ...
    def _check_criterion(self, criterion: str, context: "AgentContext") -> bool:
        ...
        criterion_checks = {
            ...
            "benefits_extracted": lambda c: (
                c.analysis_results is not None and
                len(c.analysis_results.benefits or []) > 0
            ),
            "usage_extracted": lambda c: (
                c.analysis_results is not None and
                bool(c.analysis_results.usage)
            ),
            ...
            "comparison_complete": lambda c: (
                c.analysis_results is not None and
                bool(c.analysis_results.comparison)
            ),
    ...
    def _check_parent_completion(self, parent_id: str) -> None:
        ...
        all_sub_complete = all(
            self._goals.get(sg_id, AgentGoal(id="", description="", success_criteria=[]))
    ...
    def _record_progress(
        ...
        self._goal_history.append(GoalProgress(
            goal_id=goal_id,
            progress=progress,
            status=status,
            message=message
