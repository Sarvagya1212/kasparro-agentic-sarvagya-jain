...
class DependencyGraph:
    ...
    def add_dependency(self, dep: AgentDependency) -> None:
        ...
        logger.debug(
            f"Added dependency: {dep.agent_name} depends on {dep.depends_on}"
        )
    ...
    def _group_parallel_agents(self, agents: List[str]) -> List[str]:
        ...
        for agent in agents[1:]:
            can_add = all(
                self.can_run_parallel(agent, existing)
    ...
    def visualize(self) -> str:
        ...
        for name, dep in self._dependencies.items():
            ...
            parallel = ", ".join(dep.can_parallel_with) if dep.can_parallel_with else "any"
            lines.append(f"{name}:")
...
class ParallelExecutor:
    ...
    async def run_parallel(
        ...
        for name in agents.keys():
            if name not in self._dependency_graph._dependencies:
                self._dependency_graph.add_dependency(AgentDependency(
                    agent_name=name,
                    depends_on=[],
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if total_executions > 0:
            avg_duration = sum(r.duration_ms for r in self._execution_history) / total_executions
