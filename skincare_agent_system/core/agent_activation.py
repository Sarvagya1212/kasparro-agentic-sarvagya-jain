...
class AgentActivator:
    ...
    async def wake_agent(
        ...
        if agent_name in self._last_activation:
            ...
            elapsed = (datetime.now() - self._last_activation[agent_name]).total_seconds()
            if elapsed < min_cooldown:
                logger.debug(f"Agent {agent_name} in cooldown ({elapsed:.1f}s < {min_cooldown}s)")
                return False
        ...
        logger.info(f"Queued wake request for {agent_name}: {reason} (priority: {priority})")
        return True
    ...
    def _on_trigger_event(
        ...
        asyncio.create_task(self.wake_agent(
            agent_name,
    ...
    async def _activate_agent(self, request: ActivationRequest) -> None:
        ...
        try:
            ...
            self._wake_history.append(AgentWakeRecord(
                agent_name=agent_name,
