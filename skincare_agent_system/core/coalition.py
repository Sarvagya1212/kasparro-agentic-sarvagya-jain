...
class CoalitionTask:
    ...
    dependencies: Dict[str, List[str]] = field(default_factory=dict)  # subtask -> depends_on
...
class Coalition:
    ...
    async def execute(
        ...
        logger.info(f"Coalition {self.coalition_id} executing with {len(self.members)} members")
        ...
        try:
            ...
            if hasattr(agent, 'run_async'):
                ...
                result = await loop.run_in_executor(
                    None, agent.run, context, directive
                )
...
class CoalitionManager:
    ...
    def get_active_coalitions(self) -> List[Coalition]:
        ...
        return [
            ...
            if c.status in [CoalitionStatus.FORMING, CoalitionStatus.READY, CoalitionStatus.EXECUTING]
        ]
