...
class Orchestrator:
    ...
    def run(self, context: GlobalContext) -> GlobalContext:
        ...
        while step < self.max_steps:
            ...
            if context.stage == ProcessingStage.COMPLETE:
                ...
                EventBus.emit(Events.WORKFLOW_COMPLETE, {"steps": step}, context.trace_id)
                break
            ...
            EventBus.emit(Events.AGENT_START, {
                "agent": agent.name,
            ...
            EventBus.emit(Events.AGENT_COMPLETE, {
                "agent": agent.name,
            ...
            if result.status == AgentStatus.ERROR:
                EventBus.emit(Events.AGENT_ERROR, {"message": result.message}, context.trace_id)
            ...
            if result.status == AgentStatus.VALIDATION_FAILED:
                if context.retry_count < self.MAX_RETRIES:
                    ...
                    EventBus.emit(Events.REFLEXION_TRIGGERED, {
                        "feedback": feedback,
    ...
    def _advance_stage_if_stuck(self, context: GlobalContext):
        ...
        if current_idx < len(stage_order) - 1:
            ...
            EventBus.emit(Events.STATE_CHANGE, {"new_stage": context.stage.value}, context.trace_id)
