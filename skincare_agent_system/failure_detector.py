...
class RoleComplianceChecker:
    ...
    def check_context_modification(
        self,
        agent: str,
        modified_field: str
    ) -> bool:
    ...
    def _record_violation(
        ...
        logger.error(
            f"ROLE VIOLATION: {agent} attempted '{action}' "
            f"(severity: {severity})"
        )
...
class InterAgentAuditor:
    ...
    def _log_audit(self, result: HandoffAuditResult):
        ...
        logger.info(
            f"Handoff audit {result.from_agent}→{result.to_agent}: {status}"
        )
