...
class OfflineRuleProvider(IIntelligenceProvider):
    ...
    def generate_faq(self, product_data: Dict) -> List[Tuple[str, str, str]]:
        ...
        if usage:
            ...
            questions.append(
                (f"When should I apply {name}?", f"For best results: {usage}", "Usage")
            )
        ...
        if side_effects:
            questions.append((f"Are there any side effects?", side_effects, "Safety"))
            questions.append(
                (
    ...
    def get_provider(cls) -> IIntelligenceProvider:
        ...
        if cls._online_provider.is_available():
            return CircuitBreakerWrapper(
                cls._online_provider, cls._circuit_breaker, cls._offline_provider
            )
