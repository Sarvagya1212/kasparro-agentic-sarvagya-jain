...
class InjectionDefense:
    ...
    INJECTION_PATTERNS: List[Tuple[re.Pattern, str, str]] = [
        ...
        (re.compile(r"new\s+(task|instructions?):\s*", re.I),
         "task_injection", "medium"),
        (re.compile(r"```system", re.I),
         "system_block_injection", "high"),
        (re.compile(r"\[INST\]|\[/INST\]", re.I),
         "llama_format_injection", "medium"),
        (re.compile(r"<\|im_start\|>|<\|im_end\|>", re.I),
         "chatml_format_injection", "medium"),
        (re.compile(r"<\|system\|>|<\|user\|>|<\|assistant\|>", re.I),
         "role_tag_injection", "high"),
        (re.compile(r"pretend\s+(you\s+are|to\s+be)", re.I),
         "persona_override", "medium"),
        (re.compile(r"act\s+as\s+(if|though|a)", re.I),
         "behavior_override", "medium"),
    ...
    def check_role_hijack(cls, text: str, expected_role: str) -> bool:
        ...
        for pattern in hijack_patterns:
            if re.search(pattern, text, re.I):
                logger.warning(f"Role hijack attempt detected for role: {expected_role}")
                return True
