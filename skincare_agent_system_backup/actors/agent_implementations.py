...
class DataAgent(BaseAgent):
    ...
    def propose(self, context: AgentContext) -> Optional[AgentProposal]:
        ...
        return AgentProposal(
            ...
            reason=(
                "No product data loaded - I can fetch and validate it "
...
class SyntheticDataAgent(BaseAgent):
    ...
    def propose(self, context: AgentContext) -> Optional[AgentProposal]:
        ...
        return AgentProposal(
            ...
            confidence=confidence,
            reason=(
                "Product data exists but no comparison - I can generate "
                f"Product B (conf: {confidence:.2f})"
            ),
            preconditions_met=True,
            priority=priority,
        )
    ...
    def run(
        ...
        try:
            ...
            context.log_decision(
                self.name,
                f"Generated competitor: {synthetic_product['name']} "
                f"@ ₹{synthetic_product['price']}",
            )
...
class GenerationAgent(BaseAgent):
    ...
    def propose(self, context: AgentContext) -> Optional[AgentProposal]:
        ...
        return AgentProposal(
            ...
            confidence=confidence,
            reason=(
                "Validation passed - I can generate all content pages "
                f"(conf: {confidence:.2f})"
            ),
            preconditions_met=True,
            priority=priority,
                ...
                differences = [
                    {
                        ...
                        "details": (
                            f"Common: "
                            f"{', '.join(comp['ingredients']['common_ingredients'])}."
                        ),
                    },
                    {
                        "aspect": "Price",
