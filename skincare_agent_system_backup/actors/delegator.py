...
class DelegatorAgent(BaseAgent):
    ...
    def __init__(self, name: str = "DelegatorAgent"):
        super().__init__(
            ...
            backstory=(
                "Efficient PM who coordinates workers to deliver complete analysis."
            ),
        )
        self.workers = {
    ...
    def _propose_with_llm(self, context: AgentContext, llm) -> AgentProposal:
        ...
        prompt = (
            "You are a Project Manager agent deciding whether to delegate "
            "analysis tasks.\n\n"
            "Current Context:\n"
            f"- Product: "
            f"{context.product_data.name if context.product_data else 'None'}\n"
            f"- Comparison Product: "
            f"{context.comparison_data.name if context.comparison_data else 'None'}\n"
            f"- Analysis Done: {'Yes' if context.analysis_results else 'No'}\n"
            f"- Validated: {'Yes' if context.is_valid else 'No'}\n\n"
            "Available Workers:\n"
            ...
            '    "tasks_to_run": [\n'
            '        "benefits", "usage", "questions", "comparison", "validation"\n'
            "    ]\n"
            "}"
        )
    ...
    def _propose_heuristic(self, context: AgentContext) -> AgentProposal:
        ...
        return AgentProposal(
            ...
            confidence=confidence,
            reason=(
                "Data ready - I can coordinate workers for analysis and validation "
    ...
    def run_sync(
        ...
        # PLAN: Dynamically determine functionality to execute
        required_tasks = self._plan_workflow_dynamically(context)
        context.log_decision(self.name, f"Dynamically planned tasks: {required_tasks}")
            ...
            for task_type in required_tasks:
                ...
                if best_worker:
                    ...
                    if fallback_worker:
                        context.log_decision(
                            self.name,
                            f"Task '{task_type}' assigned to {fallback_worker} "
                            f"(fallback)",
                        )
    ...
    async def run_async(
        ...
        logger.info(
            f"{self.name} ({self.role}): Starting PARALLEL delegation "
            f"with worker proposals..."
        )
        context.log_decision(
        ...
        # PLAN: Dynamically determine functionality to execute
        available_tasks = self._plan_workflow_dynamically(context)
        context.log_decision(self.name, f"Dynamically planned tasks: {available_tasks}")
        ...
        for attempt in range(self.max_retries):
            ...
            for task_type in available_tasks:
                ...
                if best_worker:
                    ...
                    logger.info(
                        f"Task '{task_type}' assigned to {best_worker} "
    ...
    def _plan_workflow_dynamically(self, context: AgentContext) -> list[str]:
        """
        Dynamically plan the workflow steps based on context and LLM reasoning.
        """
        llm = self._get_llm()
        default_plan = [
            "extract_benefits",
        ...
        prompt = (
            "You are a Project Manager agent. Plan the analysis workflow.\n"
            f"Product: {context.product_data.name if context.product_data else 'Unknown'}\n"
            f"Comparison: {context.comparison_data.name if context.comparison_data else 'None'}\n"
            "Available task types:\n"
            "- 'extract_benefits': Get product benefits\n"
            "- 'format_usage': Get usage instructions\n"
            "- 'generate_faqs': Create FAQ questions\n"
            "- 'compare_products': Compare with competitor (only if comparison data exists)\n"
            "\n"
            'Return a JSON list of task types to execute, e.g. ["extract_benefits", ...]'
        )
        ...
        try:
            ...
            if isinstance(response, list):
                ...
                return [str(t) for t in response["tasks"]]

            # If response format is unexpected
            logger.warning("LLM returned unexpected plan format. Using default.")
            return default_plan
        except Exception as e:
            logger.warning(f"Dynamic planning failed: {e}. Using default plan.")
            return default_plan
...
Why might this have failed? What should be adjusted? (1-2 sentences)"""
    ...
    def _select_worker_by_proposal(
        ...
        if best_worker:
            logger.info(
                f"Selected {best_worker} for '{task_type}' "
