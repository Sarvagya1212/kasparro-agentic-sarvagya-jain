...
class ReActLoop:
    ...
    REACT_PROMPT_TEMPLATE = (
        "You are an autonomous agent using ReAct (Reasoning + Acting) "
        "to achieve goals.\n\n"
        "GOAL: {goal}\n\n"
        "AVAILABLE ACTIONS:\n"
        "{available_actions}\n\n"
        ...
        "Instructions:\n"
        "1. First, write your THOUGHT about the current situation and what you "
        "need to do\n"
    ...
    async def reason_and_act(
        ...
        final_thought = (
            self.history[-1].thought if self.history else "No reasoning performed"
        )
    ...
    def _build_context_summary(self, context: "AgentContext") -> str:
        ...
        if context.analysis_results:
            ...
            if context.analysis_results.benefits:
                parts.append(
                    f"  Benefits: {len(context.analysis_results.benefits)} found"
                )
        else:
    ...
    def _calculate_confidence(self) -> float:
        ...
        if (
            self.history
            and self.history[-1].action
            and self.history[-1].action.upper() == "FINISH"
        ):
            base_confidence += 0.2

        # Penalize for errors in observations
        error_steps = sum(
            1 for s in self.history if s.observation and "fail" in s.observation.lower()
        )
...
# Factory function
def create_react_loop(llm_client: Any = None, max_iterations: int = 5) -> ReActLoop:
