...
class TransactionLog:
    ...
    def record(
        ...
        mutation = ContextMutation(
            mutation_id=f"mut_{self._current_version}_"
    ...
    def detect_conflicts(
        self, field_path: str, since_version: int
...
class VersionedContext:
    ...
    def _create_version(
        self, agent: str, reason: str, mutations: List[str] = None
    ) -> ContextVersion:
    ...
    def apply(
        self, changes: Dict[str, Any], agent: str = "unknown"
    ) -> "VersionedContext":
        """
        ...
        for field_path, new_value in changes.items():
            ...
            mutation = new_context._transaction_log.record(
                MutationType.SET, field_path, old_value, new_value, agent
            )
            mutation_ids.append(mutation.mutation_id)

        # Create new version
        new_context._create_version(
            agent, f"applied {len(changes)} changes", mutation_ids
        )
    ...
    def create_checkpoint(self, name: str, description: str = "") -> ContextCheckpoint:
        ...
        logger.info(
            f"Created checkpoint '{name}' at version {self._current_version_id}"
        )
        return checkpoint
    ...
    def partial_rollback(
        self, fields: List[str], to_version: int
    ) -> "VersionedContext":
        """
        ...
        for field_path in fields:
            old_value = self._get_nested_from_dict(old_version.snapshot, field_path)
            if old_value is not None:
                changes[field_path] = old_value
