...
class Orchestrator(BaseAgent):
    ...
    def _initialize_default_goals(self):
        ...
        logger.info(
            f"GoalManager initialized with {len(self.goal_manager._goals)} "
            f"default goals"
        )
    ...
    def derive_goals_from_context(self, input_context: dict = None):
        ...
        logger.info(
            f"Derived additional goals from context. "
            f"Total: {len(self.goal_manager._goals)}"
        )
    ...
    def determine_next_agent(self) -> Optional[str]:
        ...
        for p in proposals:
            self.context.log_decision(
                "Coordinator",
                f"  → {p.agent_name}: {p.action} "
                f"(confidence: {p.confidence:.2f}) - {p.reason}",
            )
        ...
        if len(proposals) > 1:
            ...
            self.context.log_decision(
                "Coordinator",
                f"Negotiation: {len(proposals)} -> {len(negotiated)} "
                f"proposals after adjustment",
            )
        ...
        if coalition:
            self.context.log_decision(
                "Coordinator",
                f"Coalition formed: {coalition['members']} "
                f"(strength: {coalition['combined_confidence']:.2f})",
            )
    ...
    def run(self, initial_product_data=None):
        ...
        self.context.log_decision(
            "Coordinator",
            f"Total proposals collected: {total_proposals} "
            f"across {len(proposal_log)} rounds",
        )
