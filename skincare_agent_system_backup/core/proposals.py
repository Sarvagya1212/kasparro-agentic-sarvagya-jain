...
class ProposalSystem:
    ...
    def collect_proposals(self, context: "AgentContext") -> List[AgentProposal]:
        ...
        logger.info(
            f"Collected {len(proposals)} valid proposals "
            f"from {len(self._agents)} agents"
        )
        return proposals
    ...
    async def collect_proposals_async(
        ...
        logger.info(
            f"Collected {len(proposals)} valid proposals "
            f"from {len(self._agents)} agents (ASYNC)"
        )
        return proposals
    ...
    def negotiate_proposals(
        ...
        for round_num in range(max_rounds):
            ...
            for proposal in current_proposals:
                ...
                if new_conf >= 0.15:
                    ...
                    logger.debug(
                        f"Proposal from {proposal.agent_name} dropped "
                        f"(conf: {new_conf:.2f})"
                    )
        ...
        logger.info(
            f"Negotiation complete: {len(proposals)} -> {len(current_proposals)} "
            f"proposals over {len(negotiation_log)} rounds"
        )
    ...
    def form_coalition(
        ...
        logger.info(
            ...
            f"(strength: {combined_confidence:.2f})"
        )
        return coalition
