...
if TYPE_CHECKING:
    ...
    from skincare_agent_system.core.proposals import AsyncEventBus
...
class PreemptionManager:
    ...
    def register_running_agent(
        self, agent_name: str, task: asyncio.Task, priority: int = 5
    ) -> None:
    ...
    def set_on_preempt_callback(
        self, agent_name: str, callback: Callable[[str], None]
    ) -> None:
    ...
    def set_on_resume_callback(
        self, agent_name: str, callback: Callable[[AgentCheckpoint], None]
    ) -> None:
    ...
    async def preempt_agent(
        ...
        self.request_cancellation(
            agent_name, reason, preempting_priority, preempting_agent
        )
        ...
        if not acknowledged:
            # Force cancel
            logger.warning(
                f"Agent {agent_name} did not acknowledge - forcing cancellation"
            )
        ...
        if self._event_bus:
            from skincare_agent_system.core.proposals import Event as PipelineEvent
    ...
    def request_cancellation(
        ...
        request = CancellationRequest(
            agent_name=agent_name, reason=reason, priority=priority, requester=requester
        )
    ...
    async def checkpoint_agent(
        ...
        if hasattr(agent, "_context") and agent._context:
            context_str = json.dumps(
                (
                    ...
                    if hasattr(agent._context, "model_dump")
        ...
        logger.info(
            f"Created checkpoint for {agent.name}: {checkpoint_id} ({progress:.1%})"
        )
    ...
    async def resume_agent(
        self, agent_name: str, context_hash: str = ""
    ...
    async def graceful_shutdown(
        self, agent_name: str, timeout: float = 5.0, create_checkpoint: bool = True
    ) -> bool:
        """
        ...
        logger.info(
            f"Initiating graceful shutdown of {agent_name} (timeout: {timeout}s)"
        )

        # Request cancellation
        self.request_cancellation(
            agent_name, "graceful_shutdown", priority=100, requester="system"
        )
    ...
    async def _wait_for_acknowledgment(self, agent_name: str, timeout: float) -> bool:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending_cancellations": sum(
                1
                for s in self._cancellation_states.values()
