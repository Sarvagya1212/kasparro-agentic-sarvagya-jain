...
class InterAgentAuditor:
    ...
    def detect_information_loss(
        ...
        for f in critical_fields:
            before_val = getattr(before, f, None)
            after_val = getattr(after, f, None)

            if before_val is not None and after_val is None:
                lost.append(f)
                logger.error(f"INFORMATION LOSS: {f} was lost during processing")
    ...
    def verify_message_integrity(self, event: "Event") -> bool:
        ...
        for f in required_fields:
            if not getattr(event, f, None):
                logger.warning(f"Event missing required field: {f}")
                return False
