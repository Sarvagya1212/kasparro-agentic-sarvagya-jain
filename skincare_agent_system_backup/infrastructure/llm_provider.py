...
class LLMCache:
    ...
    def _compute_key(
        self, prompt: str, model: str, temperature: float, system: Optional[str] = None
    ) -> str:
    ...
    def generate_json(
        ...
        if response.content:
            try:
                ...
                logger.warning("Failed to parse JSON from Anthropic")
    ...
    def generate(
        ...
        if use_cache and self._enable_cache:
            cached = self._cache.get(
                prompt,
                (
                    self._providers[target_provider].default_model
                    if target_provider
                    else ""
                ),
                temperature,
        ...
        # All failed
        return LLMResponse(content="", model="unknown", provider="none")

    def generate_json(
        self, prompt: str, system: Optional[str] = None, provider: Optional[str] = None
    ) -> Dict[str, Any]:
